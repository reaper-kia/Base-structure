...
### `GET /api/reference-data/suggestions`

Параметры: `key` — ключ реквизита, `query` — введённая пользователем строка.
...
Реальный адаптер возвращает для каждой подсказки обязательные поля `source`,
`document` и `date`. Ответ endpoint никогда не применяется к документу сам:
после выбора пользователь подтверждает значение обычным PATCH реквизитов.
Недоступность источника не влияет на создание, обработку и рендер документа.
...
```json
{"draft": "текст черновика", "doc_type": "memo", "template_id": "classic", "channel": "web"}
```

`channel` — `web` или `bot`; если поле не передано, используется `web`.
Оба канала вызывают один и тот же жизненный цикл документа.
...
| `from_registry` | пользователь явно подтвердил подсказку внешнего справочника |
...
```json
{"values": {"addressee": "Генеральному директору ООО «Ромашка» Иванову И.И."}, "from_registry": []}
```
...
Если пользователь выбрал показанную подсказку справочника, клиент добавляет её
ключ в `from_registry`; тогда значение получает статус `from_registry`. Само
наличие подсказки без этого PATCH документ не меняет.

Решение пользователя сильнее модели: повторная обработка не перезаписывает
ключи со статусом `user_provided`, `from_registry` и `left_blank`.
