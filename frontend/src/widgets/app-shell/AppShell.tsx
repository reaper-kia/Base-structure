import { Outlet, useLocation, Link } from 'react-router-dom';
import { useDocumentStore } from '../../shared/store/documentStore';

const STEPS = [
  { label: 'Черновик', sub: 'Введите текст' },
  { label: 'Настройки', sub: 'Тип и шаблон' },
  { label: 'Обработка', sub: 'ИИ-анализ' },
];

function StepBar({ current }: { current: 0 | 1 | 2 }) {
  return (
    <div className="flex items-center gap-0">
      {STEPS.map((s, i) => (
        <div key={s.label} className="flex items-center">
          <div className="flex flex-col items-center gap-0.5 px-2">
            <div
              className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all"
              style={{
                background:
                  i < current ? '#22c55e' : i === current ? 'var(--accent)' : 'rgba(255,255,255,0.15)',
                color: '#fff',
              }}
            >
              {i < current ? '✓' : i + 1}
            </div>
            <span
              className="text-xs text-center whitespace-nowrap"
              style={{ color: i <= current ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.35)' }}
            >
              {s.label}
            </span>
          </div>
          {i < 2 && (
            <div
              className="w-10 h-px mb-4"
              style={{ background: i < current ? '#22c55e' : 'rgba(255,255,255,0.2)' }}
            />
          )}
        </div>
      ))}
    </div>
  );
}

export function AppShell() {
  const location = useLocation();
  const draft = useDocumentStore((state) => state.draft);

  const current: 0 | 1 | 2 = location.pathname.startsWith('/documents')
    ? 2
    : draft.trim().length > 0
      ? 1
      : 0;

  return (
    <div
      className="min-h-full flex flex-col"
      style={{ fontFamily: 'var(--font-sans)', background: 'var(--background)' }}
    >
      <header
        style={{
          background: 'linear-gradient(to bottom, #0B3D9E, #2A8FD4)',
          boxShadow: '0 2px 16px rgba(11,61,158,0.18)',
        }}
      >
        <div
          className="max-w-4xl mx-auto px-4 md:px-6 pt-5 pb-3 flex items-center justify-between gap-4 flex-wrap border-b"
          style={{ borderColor: 'rgba(255,255,255,0.12)' }}
        >
          <div className="flex items-center gap-3">
            <div
              aria-hidden="true"
              className="flex items-center justify-center w-10 h-10 rounded-lg flex-shrink-0"
              style={{
                background: 'rgba(255,255,255,0.18)',
                border: '1.5px solid rgba(255,255,255,0.3)',
              }}
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                <rect x="3" y="3" width="8" height="8" rx="1.5" fill="white" fillOpacity="0.9" />
                <rect x="13" y="3" width="8" height="8" rx="1.5" fill="white" fillOpacity="0.6" />
                <rect x="3" y="13" width="8" height="8" rx="1.5" fill="white" fillOpacity="0.6" />
                <rect x="13" y="13" width="8" height="8" rx="1.5" fill="white" fillOpacity="0.9" />
              </svg>
            </div>
            <div>
              <div
                className="text-xs font-medium"
                style={{ color: 'rgba(255,255,255,0.55)', letterSpacing: '0.04em' }}
              >
                Портал государственных сервисов
              </div>
              <div
                className="font-bold text-white"
                style={{ fontSize: '1.05rem', letterSpacing: '-0.01em' }}
              >
                Документ за 3 шага
              </div>
            </div>
          </div>
          <StepBar current={current} />
        </div>

        <div className="max-w-4xl mx-auto px-4 md:px-6 pt-6 pb-8">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <div
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-3 text-xs font-medium"
                style={{
                  background: 'rgba(255,255,255,0.15)',
                  color: 'rgba(255,255,255,0.9)',
                  border: '1px solid rgba(255,255,255,0.2)',
                }}
              >
                <span style={{ color: '#0DC268' }}>●</span> ИИ-конструктор служебных документов
              </div>
              <h1
                className="text-2xl md:text-3xl font-bold text-white leading-tight mb-2"
                style={{ letterSpacing: '-0.02em' }}
              >
                Создайте официальный документ
                <br className="hidden md:block" /> без лишних усилий
              </h1>
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.65)', maxWidth: 460 }}>
                ИИ исправит текст, проверит реквизиты и сформирует готовый .docx-файл по выбранному шаблону
              </p>
            </div>
            <div className="flex flex-col gap-1.5 flex-shrink-0">
              {['✓ Официально-деловой стиль', '✓ Проверка реквизитов', '✓ Скачивание DOCX'].map((f) => (
                <div
                  key={f}
                  className="text-xs px-3 py-1.5 rounded-md font-medium"
                  style={{
                    background: 'rgba(255,255,255,0.12)',
                    color: 'rgba(255,255,255,0.85)',
                    border: '1px solid rgba(255,255,255,0.15)',
                  }}
                >
                  {f}
                </div>
              ))}
            </div>
          </div>
        </div>
      </header>

      <main id="main-content" className="flex-1 max-w-4xl w-full mx-auto px-4 md:px-6 py-8">
        <Outlet />
      </main>

      <footer
        className="border-t py-3 flex-shrink-0"
        style={{ borderColor: 'var(--border)', background: 'var(--card)' }}
      >
        <div className="max-w-4xl mx-auto px-6 flex items-center justify-between gap-4 flex-wrap">
          <span className="text-xs" style={{ color: 'var(--muted-foreground)' }}>
            Документ за 3 шага · ФСП Россия · 2026
          </span>
          <Link
            to="/dev"
            className="text-xs underline"
            style={{ color: 'var(--muted-foreground)' }}
          >
            Dev-панель
          </Link>
          <span
            className="text-xs"
            style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-mono)' }}
          >
            ГОСТ Р 7.0.97-2016
          </span>
        </div>
      </footer>
    </div>
  );
}