...
from ml_service.schemas import (
    FactGuardResult, ModelHealth, PredictRequest,
    PredictResponse, ProcessRequest, ProcessResponse,
)
...
def validate_llm_response(data: dict, expected_keys: list[str]) -> bool:
    ...
    if "improved_text" not in data or not isinstance(data["improved_text"], str): return False
    for k in expected_keys:
        if k not in data: return False
        if data[k] is not None and not isinstance(data[k], str): return False
    # Проверка на лишние ключи (additionalProperties = False)
    allowed = set(["improved_text"] + expected_keys)
    if any(k not in allowed for k in data.keys()): return False
    return True
...
def create_app() -> FastAPI:
    ...
    async def model_health():
        return ModelHealth(
            model_loaded=registry.is_loaded, model_version=registry.version,
            fallback_enabled=settings.fallback_enabled, supported_tasks=registry.supported_tasks
        )
    ...
    async def process_document(request: ProcessRequest):
        ...
        final_prompt = prompt_path.read_text(encoding="utf-8").format(
            doc_type_name=request.doc_type_name, structure_hint=request.structure_hint, draft=request.draft
        )
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if current_guard_result.verdict == "blocked":
                        ...
                        logger.warning(f"Fact Guard заблокировал ответ (попытка {attempt + 1})")
                        continue
        ...
        return ProcessResponse(
            request_id=request.request_id, improved_text=improved_text,
            requisites=clean_requisites, fact_guard=FactGuardResult(**current_guard_result.as_dict()), 
            is_fallback=is_fallback, reason_code=reason_code
        )
    ...
    async def predict(request: PredictRequest):
        ...
        return PredictResponse(
            request_id=request.request_id, task=request.task, predictions=predictions,
            model_version=registry.version if not fallback_used else "fallback-1.0.0",
            is_fallback=fallback_used, latency_ms=0.0,
        )
