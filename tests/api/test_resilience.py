...
def client(
    ...
    async def process_immediately(
        ...
        **_ignored_kwargs: object,
        ...
        # Роутер фиксирует состояние тумблера для текущей
...
@pytest.mark.api
def test_toggle_on_makes_processing_fail(
    client: TestClient,
) -> None:
    toggle = client.post(
        "/api/dev/break-ai",
...
@pytest.mark.api
def test_draft_survives_failure(
    client: TestClient,
) -> None:
    client.post(
        "/api/dev/break-ai",
...
@pytest.mark.api
def test_reprocess_after_toggle_off_succeeds(
    client: TestClient,
) -> None:
    client.post(
        "/api/dev/break-ai",
...
@pytest.mark.api
def test_reprocess_preserves_user_decided_requisites(
    client: TestClient,
) -> None:
    created = create_document(client)
    ...
    addressee = next(
        ...
        if requisite["key"] == "addressee"
    )
    position = next(
        requisite
        for requisite in document["requisites"]
        if requisite["key"] == "position"
    )
...
@pytest.mark.api
def test_dev_state_reflects_toggle_position(
    client: TestClient,
) -> None:
